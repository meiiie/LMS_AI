...
def _clean_runtime_scope_value(value: Any) -> str:
    return str(value or "").strip()


def runtime_cache_scope_from_context_impl(context: dict[str, Any]) -> dict[str, str]:
    """Return the request model scope that must gate semantic response cache hits."""

    provider = _clean_runtime_scope_value((context or {}).get("provider")).lower()
    model = _clean_runtime_scope_value((context or {}).get("model"))
    return {
        "provider": provider,
        "model": model,
    }
...
def _cache_entry_metadata(cache_result: Any) -> dict[str, Any]:
    entry = getattr(cache_result, "entry", None)
    metadata = getattr(entry, "metadata", None)
    return metadata if isinstance(metadata, dict) else {}


def cache_hit_matches_runtime_scope_impl(
    *,
    cache_result: Any,
    context: dict[str, Any],
) -> tuple[bool, dict[str, str]]:
    """Verify a semantic cache hit was generated with the active provider/model.

    Response cache content is model-authored. When the user pins a concrete
    runtime model, an answer generated by another model should not be replayed
    as if the current model produced it.
    """

    requested = runtime_cache_scope_from_context_impl(context)
    ...
    metadata = _cache_entry_metadata(cache_result)
    cached_provider = _clean_runtime_scope_value(
        metadata.get("generation_provider") or metadata.get("provider")
    ).lower()
    cached_model = _clean_runtime_scope_value(
        metadata.get("generation_model") or metadata.get("model")
    )
    details = {
    ...
    if requested_provider and cached_provider != requested_provider:
        return False, details
    if requested_model and cached_model != requested_model:
        return False, details
    return True, details
...
async def store_cache_response_impl(
    *,
    ...
    try:
        ...
        runtime_scope = runtime_cache_scope_from_context_impl(context)
        runtime_scope_metadata = {
            "generation_provider": runtime_scope["provider"],
            "generation_model": runtime_scope["model"],
        }
