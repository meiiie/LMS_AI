...
the control client retains the logical identity while the renderer lives; after
a reload, `session/list` remains the authority for the durable native start.
Before a new workspace can launch Codex, the application reconciles both
sources and cancels every older Codex bootstrap identity. Independent cleanup
