...
- [ ] T101 Scaffold `wiii-desktop/src/neko-chill/` subtree + `drivers/types.ts`
...
- [ ] T102 [P] `mode-store.ts`: persisted mode selection (`wiii` | `neko-chill`)
      via tauri plugin-store with localStorage fallback (FR-001); unit tests
      for persistence + default.
- [ ] T103 [P] `src-tauri/src/commands/neko_agent.rs`: process table +
...
- [ ] T201 Verify current ACP wire contract against the public schema +
...
- [ ] T202 `drivers/acp/client.ts`: JSON-RPC 2.0 correlation layer over the
...
- [ ] T203 `drivers/acp/driver.ts`: `initialize` → `session/new` →
...
- [ ] T204 `App.tsx` seam: mode check before store/auth init (early-return
...
- [ ] T301 `neko-agent-store.ts` + `AgentPicker`: consume `detect_agents`
...
- [ ] T302 `neko-session-store.ts`: session CRUD, DriverEvent→ContentBlock
...
- [ ] T303 `NekoChillApp` + `NekoTranscript` + `NekoComposer`: sidebar session
...
- [ ] T304 Wire T301–T303 end-to-end against real Gemini CLI on Windows;
...
- [ ] T401 Driver: map `session/request_permission` to a `permission-request`
      DriverEvent carrying action kind, target, and offered options; store
      pauses the turn on it (FR-006).
- [ ] T402 `PermissionCard`: explicit approve/deny surface in the transcript
      (action, target, agent identity); decision recorded as a block;
      keyboard accessible; Vietnamese-first copy.
- [ ] T403 Fail-closed paths: deny resolution reaches the agent; app-quit
...
- [ ] T604 Cross-acceptance with the partner team when `neko acp` ships:
